...
import { Calendar, CheckCircle, Clock, Loader2, Stethoscope, User, Video, X } from "lucide-react";
...
const AppointmentCard: React.FC<AppointmentCardProps> = ({
  ...
  const formatDateTime = (dateString: Date | string | null) => {
    try {
      return format(new Date(dateString as any), "MMMM d, yyyy 'at' h:mm a");
    } catch (e) {
      return "Invalid date";
    }
  };
  ...
  const formatTime = (dateString : Date | string | null) => {
    try {
      return format(new Date(dateString as any), "h:mm a");
    } catch (e) {
      return "Invalid time";
    }
  };
  ...
  useEffect(() => {
    if ((completeData as any)?.success) {
  ...
  const isAppointmentActive = () => {
    const now = new Date();
    const appointmentTime = new Date(appointment.starTime);
    const appointmentEndTime = new Date(appointment.endTime);
  ...
  const handleJoinVideoCall = async () => {
    ...
    // setAction expects string | null now
    setAction("video");
    ...
    try {
      // Guard against submitTokenRequest not being a function (defensive)
      if (typeof submitTokenRequest === "function") {
        await submitTokenRequest(formData);
      } else {
        console.warn("submitTokenRequest is not callable", submitTokenRequest);
        setAction(null);
      }
    } catch (err) {
      // on error, reset action so UI returns to normal
      console.error("Failed to request video token:", err);
      setAction(null);
    }
  };
  useEffect(() => {
    // Narrow tokenData to any so TS won't complain about optional properties
    const td = tokenData as any | undefined;

    if (td?.success && td.videoSessionId && td.token) {
      // Redirect to video call page with token and session ID
      const sessionId = encodeURIComponent(td.videoSessionId);
      const tokenStr = encodeURIComponent(td.token);
      const appointmentId = encodeURIComponent(appointment.id);

      router.push(`/video-call?sessionId=${sessionId}&token=${tokenStr}&appointmentId=${appointmentId}`);
    } else if (td?.error) {
      setAction(null);
    }
  }, [tokenData, appointment.id, router, setAction]);
  ...
  useEffect(() => {
    ...
    } else if ((tokenData as any)?.error) {
      setAction(null);
    }
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="flex items-center mt-2 text-sm text-muted-foreground">
                  ...
                  <span>{formatDateTime(appointment.starTime)}</span>
                </div>
                <div className="flex items-center mt-1 text-sm text-muted-foreground">
                  <Clock className="h-4 w-4 mr-1" />
                  <span>
                    {formatTime(appointment.starTime)} -{" "}
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center">
                  ...
                  <p className="text-white">
                    {formatDateTime(appointment.starTime)}
                  </p>
                </div>
                <div className="flex items-center">
