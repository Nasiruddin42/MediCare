...
const VerifiedDoctors = ({ doctors }: { doctors: Doctor[] }) => {
  ...
  const handleStatusChange = async (doctor: Doctor) => {
    const confirmed = window.confirm(
      `Are you sure want to suspend ${doctor.name}?`
    );
  ...
  useEffect(() => {
  if ((data as unknown as { success: boolean })?.success && targetDoctor) {
        ...
        <CardHeader>
          <div className='flex flex-col md: flex-row md:items-center justify-between gap-4'>
            ...
            <div className='relative w-full md:w-64'>
              <Search className='absolute left-2.5 h-9 w-4 text-muted-foreground'/>
              <Input
