...
interface Slot {
  startTime: string;
  endTime: string;      // added for AppointmentForm
  isAvailable: boolean;
}
...
interface SlotPickerProps {
  days: Day[];
  onSelectSlot: (slot: Slot) => void; // pass full Slot to parent
}

const SlotPicker: React.FC<SlotPickerProps> = ({ days, onSelectSlot }) => {
  ...
  const [activeTab, setActiveTab] = useState<string>(firstDayWithSlots || "");
  ...
  const handleSlotSelect = (slot: Slot) => {
    // Compute endTime assuming 30 min slot
    const start = new Date(slot.startTime);
    const end = new Date(start.getTime() + 30 * 60 * 1000);
    const fullSlot: Slot = { ...slot, endTime: end.toISOString() };
